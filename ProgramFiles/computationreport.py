...
class ComputationReport:
    ...
    @staticmethod
    def from_file(filename: str) -> 'ComputationReport | None':
        with open(filename, 'r') as f:
            try:
                return ComputationReport.from_serializable_dict(json.load(f))
            except JSONDecodeError as err:
                print(f"Can't JSON-decode {filename}.")
                return None
...
class ClientComputationReport:
    ...
    def __init__(self,
                 ...
                 timers_communication_rounds: dict[int, int] | None,
    ...
    @property
    def timers_communication_rounds(self) -> dict[int, int] | None:
    ...
    def from_folder(foldername: str) -> "ComputationReportList":
        ...
        for file in os.listdir(foldername):
            if file.startswith("computation-") and os.path.isfile(join_path_abs(foldername, file)):
                report = ComputationReport.from_file(join_path_abs(foldername, file))
