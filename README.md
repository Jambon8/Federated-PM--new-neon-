...
| `--handover-activities` | — | Path to the global handover list H, one activity per line, applied identically by every party. Required with `--use-handovers`; the prepared logs ship one as `data/2parties/<dataset>/handover.txt`, regenerated by `python3 eval/prepare/handover_lists.py` |
